...
func newAFXDPDispatcher(fd int, ep *endpoint) (linkDispatcher, error) {
	...
	// TODO(b/240191988): This is a hack. We should have a better way of
	// knowing the interface index.
	var ifaceIndex int
	interfaces, err := net.Interfaces()
	if err != nil {
		return nil, fmt.Errorf("failed to get list of interfaces: %v", err)
	}
	if len(interfaces) > 2 {
		return nil, fmt.Errorf("too many interfaces found: got %d, wanted no more than 2", len(interfaces))
	}
	...
	if ifaceIndex == 0 {
		return nil, fmt.Errorf("no suitable interface found for AF_XDP")
	}

	// Use a 2MB UMEM to match the PACKET_MMAP dispatcher.
	opts := xdp.DefaultReadOnlyOpts()
	opts.NFrames = (1 << 21) / opts.FrameSize
	dispatcher.umem, dispatcher.fillQueue, dispatcher.rxQueue, err = xdp.ReadOnlyFromSocket(fd, uint32(ifaceIndex), 0 /* queueID */, opts)
	if err != nil {
